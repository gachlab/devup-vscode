...
- **A `devup.config.mjs` now activates the extension.** It was consulted by discovery but missing from `activationEvents`, so a project whose only config was `.mjs` never woke it up.
