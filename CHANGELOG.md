...
Initial scaffold. Status bar item with live polling against the devup control plane.
...
- Command `devup: Show status` — opens a notification with detail; offers to start `devup up -d` in the integrated terminal when the daemon isn't running.
